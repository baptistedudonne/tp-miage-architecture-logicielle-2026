...
Spring instanciait le contrôleur via le constructeur sans argument, donc l'injection ne se faisait jamais et `todoItemRepository` restait `null`. La meilleure solution est l'injection par constructeur avec `@Inject` (standard JSR-330, framework-agnostique) en supprimant le constructeur vide, ce qui garantit que la dépendance est toujours fournie à l'instanciation.
...
testable unitairement sans Spring ni base de données.
