...
Adrien Burnett et Baptiste Dudonné
...
Non, tout est dans un seul package : le contrôleur, l'entité et le repository sont mélangés sans séparation des responsabilités. Cela pose plusieurs problèmes : la logique métier est couplée à la couche HTTP, le code est difficile à tester unitairement, et toute modification d'une partie peut impacter les autres.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

La règle de gestion RG1 (affichage de `[LATE!]`) se trouve dans la méthode `finalContent()` du `TodoListController`, ce qui est un anti-pattern : la logique métier devrait résider dans le domaine, pas dans la couche HTTP.

*Cette règle est-elle facilement testable par un test unitaire ?*

Non, car elle est couplée au contrôleur Spring MVC. Pour la tester, il faudrait monter un contexte Spring complet, ce qui alourdit les tests. Si elle était dans une classe domaine, un simple test JUnit sans framework suffirait.
...
`docker run` : crée et lance un nouveau conteneur. `-it` : mode interactif avec terminal (pour voir les logs en direct). `-e POSTGRES_PASSWORD=password` : définit la variable d'environnement du mot de passe PostgreSQL. `-p 5432:5432` : expose le port 5432 du conteneur sur le port 5432 de la machine hôte. `postgres` : l'image Docker officielle PostgreSQL à utiliser.

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*

Spring instanciait le contrôleur via le constructeur sans résoudre le todoItemRepository, donc l'injection ne se faisait jamais et `todoItemRepository` restait `null`. La meilleure solution est l'injection par constructeur avec `@Inject` en supprimant le constructeur vide, ce qui garantit que la dépendance est toujours fournie à l'instanciation.
...
L'architecture hexagonale place le domaine métier au centre, sans aucune dépendance vers les couches techniques. Les échanges avec l'extérieur (HTTP, base
de données) passent par des ports (interfaces) implémentés par des adapters. Les ports d'entrée (IN) exposent les use cases, les ports de sortie (OUT)
abstraient les dépendances externes. Les dépendances pointent toujours vers l'intérieur : adapters => ports => services => domain, ce qui rend le domaine
testable unitairement sans Spring ni base de données et isole la logique métier des services techniques.
...
Pour les adapters des tests d'intégration qui consistent à lancer un environnement de test (potentiellement virtuel) sont les plus adaptés.
Des tests d'isolation (via des mocks) sont possibles mais moins appropriés car ils ne testeront pas le fonctionnement technique des adapters (requête SQL, intégration dans Kafka....)
